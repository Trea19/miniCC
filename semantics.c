...
void sem_ext_def_list(ASTNode *node){
...
void sem_ext_def(ASTNode *node){
    ...
    if (strcmp(node->first_child->sibling->name, "ExtDecList") == 0){
        ...
        // struct a; ('a' hasn't been defined)
        if (!type && strcmp(node->first_child->first_child->first_child->sibling->name, "Tag") == 0){ 
...
Type* sem_struct_specifier(ASTNode *node, int wrapped_layer, int in_structure){
    ...
    if (strcmp(node->first_child->sibling->name, "Tag") == 0){
        ASTNode *tag_node = node->first_child->sibling->first_child; // Tag -> ID
    ...
    if (node->first_child->sibling->first_child){ // OptTag -> ID
...
Field_List *sem_var_dec(ASTNode *node, Type *type, int in_structure, int wrapped_layer){
    assert(node && node->first_child);
    if (!type){
        char info[MAX_ERROR_INFO_LEN];
...
Field_List *sem_def_list(ASTNode *node, int in_structure, int wrapped_layer){
    assert(node);
    if (node->first_child){
...
Field_List *sem_def(ASTNode *node, int in_structure, int wrapped_layer){
    assert(node);
...
Field_List *sem_dec_list(ASTNode *node, Type *type, int in_structure, int wrapped_layer){
    assert(node && node->first_child);
...
Field_List *sem_dec(ASTNode *node, Type *type, int in_structure, int wrapped_layer){
    assert(node && node->first_child);
    if (node->first_child->sibling){
        if (in_structure){
            char info[MAX_ERROR_INFO_LEN];
...
//        | LP Exp RP | MINUS Exp | NOT Exp | ID LP Args RP | ID LP RP 
//        | Exp LB Exp RB | Exp DOT ID | ID | INT | FLOAT
    ...
    if (strcmp(node->first_child->name, "Exp") == 0){
        if (strcmp(node->first_child->sibling->name, "ASSIGNOP") == 0){
            ...
            if (check_equal_type(type1, type2)){
                //check left-value
                ASTNode *child_node = node->first_child;
...
Field_List* query_field_hash_table(unsigned hash_index, char *str, ASTNode *node, int look_for_structure){
...
Field_List* insert_field_hash_table(unsigned hash_index, char *str, Type *type, ASTNode *node, int wrapped_layer, int is_structure){
...
// params: error-type, error-info, line-num
